...
##
# Sep 5, 21 - set up the path directory to run on A & C only (b excluded), merged truncated to 250 - 265 (V4 is 254 in bacteria)
...
## April 22, 2022: added script details, section dividers and removed unecessary lines  
##     
##
## ---------------------------
...
## below can be used to run R module on Queen's Universities Centre for Advanced Computation high-performance computing (HPC) clusters but do not use if you are submitting this script to the SLURM scheduler
...
path <- here("~/Raw_data/Tick16sMiSeq2019/MiSeq_from_David/2021_Aug_cutadapt_demult_spacer_e0.1/Renamed/To_download_2021Aug20/A_C_libraries_only") # change the absolute path to the directory containing the A and C replicate fastq files
...
## sep5_21 addition need to re-set the path the the parent directory so the script can access the references and save this in the 'test_data' folder

path <- here("~/Raw_data/Tick16sMiSeq2019/MiSeq_from_David/2021_Aug_cutadapt_demult_spacer_e0.1/Renamed/To_download_2021Aug20") 

filtFs_trunc_trim <- file.path(path, "filtered_trim_sep5_no_b_noprim", paste0(sample.names, "_F_filt_trim_sep5_no_b_noprim.fastq.gz"))
filtRs_trunc_trim <- file.path(path, "filtered_trim_sep5_no_b_noprim", paste0(sample.names, "_R_filt_trim_sep5_no_b_noprim.fastq.gz"))
...
## filter the merged data for sequences of lengths ranging from 250-265 bp
...
saveRDS(seqtab.nochim, here("test_data/seqtab.nochim_sep5_job11_AandC_only.RDS")) 
...
saveRDS(taxa, here("test_data/taxa_test_sep5_job11_AandC_only.RDS"))
